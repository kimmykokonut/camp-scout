...
def main():
    ...
    while True:
        ...
        if campsites_info:
            ...
            print(message)
            start_polling = input(
                "\n Do you want me to continue scanning for {selected_campground_name} in {month_name}?"
            )
            if start_polling:
                user_email = input(
                    "Enter your email and I will notify you when new spots open up..."
                )
                print("To quit polling, type ctrl + c")
                # poll here
            else:
                break
        else:
